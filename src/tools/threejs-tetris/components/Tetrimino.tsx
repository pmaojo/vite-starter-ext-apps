 ...
 * 单独一个方块
 */
export const Tetrimino: React.FC<{ block: Block; color: string }> = React.memo(
  ({ block, color }) => {
    return (
 ...
 * 所有方块构成的整体
 */
export const TetriminoGroup: React.FC<TetriminoProps> = React.memo(
  ({ type, position, blocks, scale = 1 }) => {
 ...
 * 已经下落的方块集合
 */
export const TetriminoPile: React.FC<{ grid: (string | null)[][][] }> =
  React.memo(({ grid }) => {
    const tetrimino = [];
